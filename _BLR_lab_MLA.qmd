...
::: {.callout-important icon=false title="Question"}
Based on the likelihood ratio test, does the `overall_satisfaction × WalkScore` interaction significantly improve model fit?
:::
...
* `lmer.df = "satterthwaite"` specifies the method for computing degrees of freedom and confidence intervals, using Satterthwaite's approximation which is appropriate for mixed models
...
In this lab, we have used two different approaches for testing fixed effects:
