...
To compare the mean change in body weight across the three treatment groups, we use the one-way ANOVA model:
...
- Using *effects coding* with Pharmacological as the implied category under the sum-to-zero constraint, the statistically equivalent regression model is:
...
- Treatment deviations:
  ...
  - $\tau_{\text{Lifestyle}}   = \beta_2$
  - $\tau_{\text{Pharmacological}}   = -(\tau_{\text{Control}} + \tau_{\text{Lifestyle}}) = -(\beta_1 + \beta_2)$
...
- In more complex models with multiple factors, EMMs provide predicted group means that are averaged over the levels of these additional factors
...
- Hypotheses
  - $H_0$: all group means or equal ($\tau_{Control} = \tau_{Lifestyle} = \tau_{Pharmacological} = 0$)
