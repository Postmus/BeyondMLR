...
- The total variation in the response can be decomposed as
  ...
  - $SS_{Treatment}$: variation explained by differences between treatment group means (between-group variation)
  - $SS_{Residual}$: unexplained variation within treatment groups (within-group variation)
  
- A commonly used effect-size for ANOVA is eta-squared, which measures the proportion of total variability explained by the treatment:

  $$\eta^2 = \frac{SS_{Treatment}}{SS_{Total}}$$

  - Interpretation: the fraction of total variability attributable to treatment differences; larger values indicate a stronger effect
  - Note: in a one-way ANOVA this equals the percentage of variance explained (multiply by 100)
...
- The F statistic compares the variance explained by treatment to the residual variance:
  ...
  - Under $H_0$, $F$ follows an $F$-distribution with $df_{1}=k-1$ and $df_{2}=N-k$, where $k$ is the number of treatment groups and $N$ is the total sample size
...
anova_df[num_cols] <- lapply(anova_df[num_cols], function(x) round(x, 3))
cat(knitr::kable(anova_df, format = "html", table.attr = 'class="small-anova"'))
```
...
anova_df1 <- as.data.frame(anova(fit_oneway))
num_cols1 <- sapply(anova_df1, is.numeric)
anova_df1[num_cols1] <- lapply(anova_df1[num_cols1], function(x) round(x, 3))
html1 <- knitr::kable(anova_df1, format = "html", table.attr = 'class="small-anova"')
cat(html1)
```
:::
...
anova_df2 <- as.data.frame(anova(fit_adj))
num_cols2 <- sapply(anova_df2, is.numeric)
anova_df2[num_cols2] <- lapply(anova_df2[num_cols2], function(x) round(x, 3))
html2 <- knitr::kable(anova_df2, format = "html", table.attr = 'class="small-anova"')
cat(html2)
```
:::
