...
R output is a bit smaller compared to the ont size for the text. Is there a way to control that
...
- A one-way ANOVA can be written as a linear regression with a categorical predictor, which lets us apply regression toolbox to estimate and test effects
...
- Estimated marginal means (EMMs) are the model-based estimates of the group means obtained from the fitted model
- In a one-way ANOVA they coincide with the observed group means; in more complex models they are the model-predicted means averaged over other factors

```{r}
#| echo: false
# Obtain estimated marginal means from the fitted lm object
library(emmeans)
...
- Hypotheses:
 ...
 - We test this using the F-test shown in the ANOVA table:
...
- The p-value is $\leq 0.05$, so we reject $H_0$ and conclude that treatment has a significant effect on blood pressure reduction

## Post-hoc comparisons

- Since we found a significant treatment effect, we will conduct pairwise comparisons to identify which specific treatment groups differ significantly from one another
...
::: {.callout-note icon=true title="What are contrasts"}
- A contrast is a pre-specified linear combination of group means used to test a focused hypothesis:

  $$L=\sum_{i=1}^k c_i\mu_i, \qquad \sum_{i=1}^k c_i = 0.$$

- Examples:
  - Compare A vs B: $c=(1,-1,0)$ so $L=\mu_A-\mu_B$.
  - A vs average of B and C: $c=(1,-\tfrac{1}{2},-\tfrac{1}{2})$ so $L=\mu_A-\tfrac{1}{2}(\mu_B+\mu_C)$.

- Testing: estimate $\hat L$ from the fitted model and compute $t = \hat L / \mathrm{SE}(\hat L)$; test whether $L = 0$
- Note: contrasts can also be written as linear functions of regression coefficients. Since the vector of group means can be written as $\mu = M\beta$ for a suitable matrix $M$, any contrast $L = c'\mu$ can be rewritten as $L = (M'c)'\beta$ and tested directly on $\beta$.
:::
