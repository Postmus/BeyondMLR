...
- Context variables are factors that define shared conditions under which observations are made but that are not of primary interest
...
- Objective: test the toxic effects of an experimental drug X by assessing changes in body weight in rats
- Study design:
...
## Model 1: one-way ANOVA ignoring cage
...
## Model 2: two-way additive ANOVA

:::: {.columns}

::: {.column width="50%"}

**Cage as fixed effect**

$$
Y_{ijk} = \mu + \tau_i + \beta_j + \varepsilon_{ijk}
$$

- $\beta_j$: fixed parameter describing how cage $j$ differs from the overall mean
- Adjusts for each cage separately; model complexity increases with the number of cages
...
- $b_j$: random intercept capturing cage-to-cage variation around the overall mean
- Estimates the average variability between cages ($\sigma_b^2$), not each cage’s deviation from the overall mean separately
...
- Both models statistically adjust for the effect of cage, separating it from the residual variance ($\sigma^2_\varepsilon$) 
- In balanced designs, both yield identical estimates and inference for the treatment effect
- In unbalanced designs or when interaction terms are included, the random-effects model provides more robust treatment estimates because it borrows strength across cages and is less sensitive to uneven group sizes
- For these reasons, random effects are generally preferred when the context variable (e.g., cage, laboratory, center) represents a random sample from a larger population of possible contexts
...
```{r}
re <- tidy(m3, effects = "ran_pars")[, c("group", "term", "estimate")]
...
- In the one-way ANOVA that ignored cage, all unexplained variation was captured by a single residual error term ($\sigma^2_{\text{total}}$)
- In the two-way ANOVA with cage as a random effect, that unexplained variability is partitioned into:
  1. Between-cage variance ($\sigma^2_{\text{cage}}$): differences in average weight change between cages
  2. Within-cage (residual) variance ($\sigma^2_{\text{residual}}$): differences among rats within the same cage after accounting for treatment and cage effects
- Together, these components satisfy: $\sigma^2_{\text{total}} = \sigma^2_{\text{cage}} + \sigma^2_{\text{residual}}$

<div style="position: relative; left: -10%; width: 60%; text-align: left; margin-top: 0; margin-bottom: 0;">
