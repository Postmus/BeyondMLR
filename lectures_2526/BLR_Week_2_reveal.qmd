...
# Part 2: Cross-classified random-effects models
...
- When observations are influenced by multiple context variables, different grouping structures can occur
  - Hierarchical: one context variable is nested within the other (discussed in weeks 3 and 4)
    - Example: pupils (level 1) nested within classes (level 2), and classes nested within schools (level 3)
    - Example: longitudinal measurements (level 1) nested within patients (level 2), and patients nested within clinics (level 3)
...
- To separate these sources of variability, we use a **cross-classified random-effects model**
