...
- Randomized block design:
  - Estimates the fixed effect of treatment while accounting for block-to-block variability
  - The blocking variable is modeled as a random effect

- Replicated randomized block design:
  - Extends the basic design by adding replicates within each block
  - Allows inclusion of a random treatment–block interaction term
  - Although this interaction is not of direct interest, modeling it gives a more realistic representation of uncertainty in the average treatment effect and yields more conservative inference when the treatment–block interaction ICC is large
    - More conservative: lower probability of declaring a treatment effect when none exists (better control of Type I error)
...
- Crossed random-effects models require replication in data collection
  - Each patient must be assessed by multiple radiologists (and vice versa)
  - This crossed design allows us to estimate separate variance components for each source of variability
  - Without such replication, we cannot disentangle the effects of different random factors

- Crossed random-effects models are particularly valuable in medical research for assessing inter-rater agreement and consistency

- Interactions between crossed random effects are also possible
  - This would require multiple measurements per patient-radiologist pair
  - For example: each radiologist measures each tumor twice, allowing us to estimate a patient-by-radiologist interaction capturing whether consistency between raters varies across patients
