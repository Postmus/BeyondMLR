...
## What did we learn last week - part 1

#### One-way ANOVA

- Dummy coding and effects coding are two schemes for incorporating nominal variables (unordered factors in R) into linear regression models
  ...
  - ANOVA tests, estimated marginal means, and post-hoc comparisons are invariant to the coding scheme

#### Feedback assignment 1

- No major difficulties were encountered in the submitted reports and associated Quarto files
  - Analyses and interpretations were generally correct
  - Students demonstrated appropriate use of the `dplyr` and `ggplot2` packages for creating summary tables and graphs

## What did we learn last week - part 2
...
- As a result, observations within the same block are positively correlated, violating the independent errors assumption underlying ordinary linear regression 
  - Applying ordinary linear regression in these situations gives unbiased treatment effect estimates but overestimates the corresponding standard errors, reducing the power to detect a treatment effect
- The independent errors assumption can be relaxed by including block as a random effect, which decomposes the unexplained variation (i.e., variation not explained by treatment) into:
  - Variation between blocks (due to shared conditions)
  - Residual variation between individual units within blocks
- Compared to ordinary linear regression, this approach increases the precision with which the treatment effect is estimated, resulting in greater statistical power
...
# Part 1: Two-way factorial ANOVA
...
2. Test the interaction effect
    - If the interaction effect is significant
      ...
      - Estimate and test conditional means, applying a Bonferroni adjustment for pairwise comparisons within levels
...
## Reporting the results - worked example
...
- As discussed previously, the tests for the main effects assess whether the average effect of each factor, averaged over all levels of the other factor, differs from zero
  - Main effect of composite: are the row means different?
  - Main effect of curing time: are the column means different?
- When an interaction is present, a simple effects analysis is more informative, examining how one factor affects the response within each level of the other factor
...
A two-way factorial ANOVA revealed a significant interaction between composite type and curing time, F(2, 54) = 26.72, p < 0.001. Given this interaction, a simple-effects analysis was conducted to examine the effect of curing time within each composite type. For hybrid composite, curing time significantly affected hardness (p < 0.001): mean hardness was higher after 60-second curing (52.1, 95% CI: 51.2–53.0) than after 20-second curing (47.0, 95% CI: 46.1–47.8). For microfine composite, there was no significant difference between 20-second curing (49.7, 95% CI: 48.8–50.5) and 60-second curing (49.5, 95% CI: 48.6–50.3), p = 0.739. For microhybrid composite, curing time likewise had no significant effect, with means of 50.7 (95% CI: 49.9–51.6) for 20-second curing and 50.3 (95% CI: 49.4–51.1) for 60-second curing, p = 0.460.

## Lab 3: Two-way factorial ANOVA
...
- Two-way factorial ANOVA can be used to test the effects of two experimental factors and their interaction:
  - When the interaction is not significant, proceed with testing the main effects
...
- In part 1, we examined interactions between two fixed effects in a two-way factorial ANOVA
  
- It is also possible to have interactions between a fixed effect and a random effect
  - Example: in a randomized block design, treatment is a fixed effect and blocks are a random effect
  - A treatment-by-block interaction means that the treatment effect varies across blocks

- When we model a treatment-by-block interaction, the interaction itself becomes a random effect
  - Rather than estimating a separate interaction parameter for each block, we estimate the variance of the treatment-by-block interaction
    - This variance captures how much the treatment effect fluctuates across the population of blocks
    - High variance indicates that treatment effectiveness is inconsistent across contexts
    - Low variance indicates that treatment effects are stable and generalizable
...
- Randomized block design:
  - Estimates the fixed treatment effect while accounting for block-to-block variability by modeling 
  the blocking variable as a random effect
  - Generally provides greater power than ordinary linear regression by accounting for within-block correlation

- Replicated randomized block design:
  - Extends the basic design by including multiple replicates within each block
  - Allows estimation of a random treatment-by-block interaction term
  - Although this interaction is not of direct interest, modeling it provides a more realistic representation of uncertainty in the average treatment effect
  - Yields more conservative inference when the treatment-by-block interaction ICC is large
    - More conservative: lower probability of falsely declaring a treatment effect (better Type I error control)
    - Despite being more conservative, still generally provides greater power than ordinary linear regression that ignores the blocking structure

:::

# Part 3: Crossed random-effects models

## Hierarchical and crossed grouping structures

- Grouped data arise when observational units share common conditions defined by one or more context variables
- So far, we have considered data structures with a single context variable, such as the blocks in a randomized block design
  - This creates a two-level hierarchical structure, with the observational units at the lower level (level 1) and the levels of the context variable at the higher level (level 2)
- When observational units are influenced by multiple context variables, different grouping structures can occur
  - Hierarchical: one context variable is nested within another (discussed in subsequent weeks)
    - Example: pupils (level 1) nested within classes (level 2), and classes nested within schools (level 3)
  - Crossed: lower-level units belong to combinations of two (or more) non-nested higher-level units
    - Example: exam grades (level 1) cross-classified by student (level 2) and teacher (level 2)
    - In medical research, crossed structures often arise when assessing inter-rater agreement

## Example: agreement among radiologists
...
- Agreement measures the extent to which radiologists give the same numerical results when assessing the same patients  
...
- Crossed random-effects models extend beyond hierarchical structures by including two (or more) non-nested random effects
  
- Crossed random-effects models are commonly used in medical research for assessing inter-rater agreement when the outcome is measured on a continuous scale
  - For binary outcomes, agreement is typically assessed using Cohen's kappa or Fleiss' kappa

- Interactions between crossed random effects are also possible:
  - This requires multiple measurements per patient-radiologist pair
  - For example: each radiologist measures each tumor twice, allowing estimation of a patient-by-radiologist interaction that captures whether consistency between raters varies across patients  
