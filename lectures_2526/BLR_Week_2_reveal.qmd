...
- The tests for the main effects assess whether the average effect of each factor, averaged over all levels of the other factor, differs from zero
...
- Replicated randomized block design:
  ...
  - Although this interaction is not of direct interest, modeling it better reflects uncertainty in the average treatment effect and typically yields more conservative inference
- Interactions between fixed effects (time) and random effects (subject) are also common in longitidinal data analysis (discussed in week 4)
...
- When observational units are influenced by multiple context variables, different grouping structures can occur
  - Hierarchical: one context variable is nested within another (discussed in weeks 3 and 4)
