...
- Longitudinal data: repeated measurements on the same subjects over time
- Key feature: observations within the same subject are correlated
- Goal: understand patterns of change over time and factors that influence them

**Topics covered**

1. Introduction to the growth data example
2. Exploratory data analysis for longitudinal data
3. Random intercept model for longitudinal data
4. Extension to random slopes
5. Model building strategy (to be continued)

:::

## Longitudinal data as multi-level data

::: {style="font-size: 0.85em;"}

**Cross-sectional multi-level data** (last week)

- Subjects nested within groups (e.g., patients in hospitals, listings in neighborhoods)
- Between-group differences: groups differ in their average outcomes
- Within-group correlation: subjects in the same group tend to be similar

**Longitudinal data** (this week)

- Measurement occations nested within subjects
- Between-subject differences: individuals differ in their baseline levels
- Within-subject correlation: repeated measurements on the same person are correlated
- **New element**: individuals may also differ in their *rate of change* over time

**Longitudinal data is multi-level data**

- In longitudinal data, subjects are the "groups"
- Time is a level-1 variable (varies within subjects)
- Subject characteristics are level-2 variables (constant within subjects)
...
- The mean trajectory shows consistent growth over the first year

**Individual variation**

- **Variation in baseline**: some boys start taller, others shorter (different intercepts)
- **Variation in growth rate**: some boys grow faster than others (different slopes)
- Trajectories remain roughly parallel but with clear individual differences
...
## Examining individual trajectories
...
set.seed(123)
sampled_ids <- sample(unique(growth$id), 16)
...
growth |>
  ...
  labs(
    title = "Sample of Individual Growth Trajectories (16 boys)",
    x = "Age (weeks)",
    y = "Height (cm)"
  ) +
  theme_minimal() +
...
## Observations from individual trajectories

**What the facet plot reveals**

- Each boy shows a clear growth pattern over time
- **Baseline differences**: Starting heights vary across individuals
- **Slope differences**: Growth rates (steepness of lines) vary across individuals
  - Some boys have steeper trajectories (faster growth), others flatter (slower growth)

**Measurement frequency**

- Measurements are taken approximately weekly
- Some variation in the exact timing of measurements
- Most boys have complete follow-up across the study period
...
- This high ICC confirms that repeated measurements within the same boy are strongly correlated
- The remaining `r round((1 - icc) * 100)`% represents within-person variation (measurement occasion differences)

:::

## ICC in longitudinal versus cross-sectional data

**Why do we include time when calculating the ICC?**

- In longitudinal data, the mean of the outcome is itself a function of time
- We therefore want to assess variation about the **time-specific mean**, not the overall mean
...
Looking at our spaghetti plot:

- Some boys clearly grow faster than others
- Trajectories are not perfectly parallel
- The spread of trajectories increases over time
...
- A random slope allows the effect of a predictor (here: age) to vary across individuals
...
::: {style="margin-top: 1.0em; font-size: 0.9em;"}
...
**ICC is no longer constant**

With random slopes, the between-person variance changes over time:

$$\text{Var}(b_{0i} + b_{1i} \cdot \text{age_c}) = \sigma^2_{b0} + 2 \cdot \text{age_c} \cdot \sigma_{b01} + \text{age_c}^2 \cdot \sigma^2_{b1}$$

The ICC at a given time point is:
...
- As time moves away from the mean, the ICC changes depending on:
  - The variance of random slopes ($\sigma^2_{b1}$)
  - The covariance between intercepts and slopes ($\sigma_{b01}$)
...
Before adding covariates, we should check whether the time trend is correctly specified.
...
- If the loess curve deviates systematically from zero → linear time trend may be inadequate
  - U-shape suggests quadratic growth
- If the spread of residuals changes with fitted values → heteroscedasticity
...
The U-shaped residual pattern suggests growth is not perfectly linear. We extend the model:
...
**Why fixed effect only?**

- A random quadratic slope ($b_{2i}$) is possible in theory, but adds complexity:
  - Three more parameters: $\sigma^2_{b2}$, $\text{Cov}(b_0, b_2)$, $\text{Cov}(b_1, b_2)$
  - Risk of convergence problems and overfitting with limited data
- Start simple: add quadratic as fixed, check if it resolves the residual pattern
- Only add random quadratic if there's strong evidence of individual differences in curvature
...
- If random slope variance is small / non-significant → simplify to random intercept only
...
- Convenient for testing multiple fixed effects simultaneously
...
- More flexible: can compare any two nested models
- Once final structure is determined, refit with REML for parameter estimation

**Which to use?**

- For simple tests (e.g., "is the quadratic term needed?"): either approach works
...
## EDA: growth trajectories by breastfeeding status
...
- A 1 cm difference in height at 6 months is well within normal variation
