...
## Assignment week 3: what went well

::: {style="font-size: 0.8em;"}

**Strong foundations**

- Most submissions included good exploratory data analysis with appropriate visualizations
- Nearly all correctly calculated and interpreted the ICC from the null model
- Model building generally followed a logical sequence: null → L1 → L2 → interactions
- Appropriate use of effects coding and centering of continuous predictors

**Good modeling practices**

- Correct specification of random effects structure
- Proper use of `lmerTest` for obtaining p-values
- Many submissions included thoughtful interpretation of fixed effects
- Cross-level interactions were generally specified and interpreted correctly

**Effective communication**

- Clear narrative structure connecting research questions to model choices
- Good use of `emmeans` for visualizing interactions
- Tables and figures were generally well-formatted and informative

:::

## Assignment week 3: areas for improvement

::: {style="font-size: 0.85em;"}

**Model diagnostics**

- Many submissions did not include residual diagnostics (residuals vs. fitted plot)
- The data had a **floor effect** (many zero scores) causing heteroscedasticity
  - This is detectable with a simple diagnostic plot — always check your assumptions!
  - Transformations don't fully solve floor effects since both log(0+c) and sqrt(0) still cluster at the boundary

**Statistical verus practical significance**

- With n = 15,362, even tiny effects are statistically significant
- Always consider effect sizes: a significant p-value does not mean the effect matters
- Ask: "Is this difference meaningful in the real-world context?"

**Other points of attention**

- Some analyses were inadvertently run on a subset of the data (e.g., a sample created for EDA/plotting) rather than the full dataset: always verify your model uses the complete data

:::

## When to transform the outcome variable

::: {style="font-size: 0.8em;"}

**Common misconception**: "The outcome must be normally distributed, so I need to transform it"

- Regression does **not** assume that the outcome is normally distributed; the assumption applies to the **residuals**
- A skewed outcome can yield perfectly normal residuals if predictors explain the skewness

**Let the diagnostics guide you**

- Check residuals vs. fitted values plot for heteroscedasticity and non-linearity
- If diagnostics look fine → no transformation needed, regardless of outcome distribution

**When log-transformation may help**

- Heteroscedasticity where variance increases with the mean
- Especially likely when the underlying process is **multiplicative** (e.g., prices, incomes, concentrations)
- Effects are naturally proportional: “a nice view adds 20%” not “adds €20”

**When transformation will not help**

- **Floor/ceiling effects** from measurement limits (e.g., many zero exam scores)
- Boundary clustering persists because log(0+c) and sqrt(0) still map to (near) zero

:::

## Lab week 3: Airbnb listing prices

```{r}
#| fig-width: 10
#| fig-height: 6

# Load the Chicago Airbnb data
...
# Fit null models
null_model_raw <- lmer(price ~ (1 | neighborhood), data = chicago_airbnb)
null_model_log <- lmer(log_price ~ (1 | neighborhood), data = chicago_airbnb)

# Create 2x3 grid
p1 <- ggplot(chicago_airbnb, aes(x = price)) +
  geom_histogram(bins = 30, fill = "skyblue", color = "black") +
  labs(title = "Untransformed price", x = "Price (USD)", y = "Frequency") +
  theme_minimal()

p2 <- ggplot(chicago_airbnb, aes(x = fitted(null_model_raw), y = residuals(null_model_raw))) +
...
p3 <- ggplot(data.frame(resid = residuals(null_model_raw)), aes(x = resid)) +
  geom_histogram(bins = 30, fill = "skyblue", color = "black") +
  labs(title = "Residual distribution", x = "Residuals", y = "Frequency") +
  theme_minimal()

p4 <- ggplot(chicago_airbnb, aes(x = log_price)) +
  geom_histogram(bins = 30, fill = "lightgreen", color = "black") +
  labs(title = "Log-transformed price", x = "Log(Price)", y = "Frequency") +
  theme_minimal()

p5 <- ggplot(chicago_airbnb, aes(x = fitted(null_model_log), y = residuals(null_model_log))) +
  geom_point(alpha = 0.2) +
  geom_smooth(method = "loess", se = FALSE, color = "steelblue", linewidth = 1) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  labs(title = "Residuals vs. fitted", x = "Fitted values", y = "Residuals") +
...
p6 <- ggplot(data.frame(resid = residuals(null_model_log)), aes(x = resid)) +
  geom_histogram(bins = 30, fill = "lightgreen", color = "black") +
  labs(title = "Residual distribution", x = "Residuals", y = "Frequency") +
  theme_minimal()
...
Log transformation stabilizes variance and improves residual normality

## Assignment week 3: Exam scores
...
ilea_data <- ilea_data |>
  mutate(across(where(is.character), as.factor),
         ...
         log_exam = log(ExamScore + 1))

# Fit null models
null_model_raw <- lmer(ExamScore ~ (1 | School), data = ilea_data)
null_model_log <- lmer(log_exam ~ (1 | School), data = ilea_data)

# Create 2x3 grid
p1 <- ggplot(ilea_data, aes(x = ExamScore)) +
  geom_histogram(bins = 30, fill = "skyblue", color = "black") +
  labs(title = "Untransformed exam score", x = "Exam Score", y = "Frequency") +
  theme_minimal()

p2 <- ggplot(ilea_data, aes(x = fitted(null_model_raw), y = residuals(null_model_raw))) +
  geom_point(alpha = 0.2) +
  geom_smooth(method = "loess", se = FALSE, color = "steelblue", linewidth = 1) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  labs(title = "Residuals vs. fitted", x = "Fitted values", y = "Residuals") +
...
p4 <- ggplot(ilea_data, aes(x = log_exam)) +
  geom_histogram(bins = 30, fill = "lightsalmon", color = "black") +
  labs(title = "Log-transformed", x = "log(Exam Score + 1)", y = "Frequency") +
  theme_minimal()

p5 <- ggplot(ilea_data, aes(x = fitted(null_model_log), y = residuals(null_model_log))) +
  geom_point(alpha = 0.2) +
  geom_smooth(method = "loess", se = FALSE, color = "steelblue", linewidth = 1) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
  labs(title = "Residuals vs. fitted", x = "Fitted values", y = "Residuals") +
...
Floor effect persists: boundary clustering remains visible in both residual plots after transformation

## [Longitudinal data analysis: Growth curves example]{style="font-size: 0.9em;"}

```{r}
# Load the growth data
growth <- read_sav("../data/growth.sav")
...
# Use effects coding for categorical predictors
...
- Breastfeeding (`bf`) remains with a main effect and interaction with `age_c`:
  - **Main effect**: Exclusively breastfed babies are ~0.5 cm below the grand mean at mean age
  - **Interaction**: Exclusively breastfed babies grow ~0.015 cm/week slower than average
