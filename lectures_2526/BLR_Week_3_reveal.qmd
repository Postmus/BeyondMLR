...
- When the ICC is large, wards vary substantially in their baseline performance
  - Any observed treatment effect could partly reflect chance allocation of the intervention to better-performing wards, rather than a true intervention effect
  - With only 12 wards randomized, we have limited ability to distinguish "lucky randomization" from true treatment benefits
...
- The mixed model accounts for this uncertainty by producing larger (and correct) standard errors and wider confidence intervals
...
- Accounting for blocks **increases precision**
  - Reduces residual variance by removing block-to-block variation
  - Smaller standard errors for treatment effects
  - More statistical power to detect treatment differences
- Ignoring blocks is **inefficient but valid**
  ...
  - No inflation of Type I error rates
:::
...
- Accounting for clusters **decreases precision**
  ...
  - Larger standard errors for treatment effects
  - Less statistical power compared to individual randomization
- Ignoring clusters is **invalid**
  - Treatment effect estimates remain unbiased
...
- Research questions:
  ...
  - Which patient-level and hospital-level factors are associated with length of stay?
...
- This unconditional ICC reflects the correlation in length of stay among patients from the same hospital, measured before adjusting for any patient or hospital characteristics
...
- Adding patient-level characteristics typically reduces residual variance (within-hospital variation) and may also reduce hospital variance if hospitals differ in their patient mix
...
## Variance components: model comparison
...
```{r}
#| echo: false
# Extract variance components from all three models
vc_comparison <- data.frame(
  Model = c("Null model", "Null model", "Patient model", "Patient model", "Full model", "Full model"),
  Component = rep(c("Hospital", "Residual"), 3),
  Variance = c(
    as.data.frame(VarCorr(model_null))$vcov[1],
    as.data.frame(VarCorr(model_null))$vcov[2],
    as.data.frame(VarCorr(model_patient))$vcov[1],
    as.data.frame(VarCorr(model_patient))$vcov[2],
    as.data.frame(VarCorr(model_full))$vcov[1],
    as.data.frame(VarCorr(model_full))$vcov[2]
  )
)
...
```{r}
#| echo: false
# Calculate ICCs for each model
icc_null <- as.data.frame(VarCorr(model_null))$vcov[1] / sum(as.data.frame(VarCorr(model_null))$vcov)
icc_patient <- as.data.frame(VarCorr(model_patient))$vcov[1] / sum(as.data.frame(VarCorr(model_patient))$vcov)
icc_full <- as.data.frame(VarCorr(model_full))$vcov[1] / sum(as.data.frame(VarCorr(model_full))$vcov)

icc_table <- data.frame(
  Model = c("Null model", "Patient model", "Full model"),
  ICC = c(icc_null, icc_patient, icc_full)
)
...
- **Unconditional ICC** (null model) = `r round(icc_null, 3)`: `r round(100 * icc_null, 0)`% of total variance in length of stay is attributable to differences between hospitals before adjusting for any covariates

- **ICC after patient adjustments** = `r round(icc_patient, 3)`: `r round(100 * icc_patient, 0)`% of remaining variance is at the hospital level after accounting for patient characteristics

- **Conditional ICC** (full model) = `r round(icc_full, 3)`: `r round(100 * icc_full, 0)`% of unexplained variance remains at the hospital level after accounting for all covariates

## Unconditional versus conditional ICC

- Unconditional ICC
  - Computed from a null model without covariates
  - Captures the raw correlation among individuals in the same cluster
  - Useful for describing the baseline dependence structure in hierarchical data

- Conditional ICC
  - Computed from a model that includes covariates
  - Reflects the remaining clustering after adjustment
  - Affects precision differently for different types of fixed effects
    - Group-level (level 2) covariates: a higher ICC reduces precision
    - Individual-level (level 1) covariates: a higher ICC increases precision
    - Cross-level (level 1 x level 2) interactions: a higher ICC can raise or lower precision depending on whether the interaction is affected more by variation within hospitals or by differences between hospitals

## Lab 5: multi-level analysis Chicago AirBnB data
