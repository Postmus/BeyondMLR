...
## What did we learn in previous weeks
...
#### What do we mean by within-group correlation?

- In the familiar notion of correlation, we examine how two variables (X and Y) co-vary within the same unit
- In multi-level data, correlation refers to the similarity between outcomes that belong to the same group
- A useful thought experiment for two-level hierarchical data
  ...
  - Groups with higher (or lower) group means will consistently produce pairs of outcomes that both tend to be above (or below) the grand mean
  - If we repeat this process many times and plot each pair of outcomes on a scatterplot, the degree to which points cluster around the diagonal reveals the strength of the correlation structure
- The ICC quantifies the strength of this within-group correlation  
...
- Randomized block design
   - Two-level hierarchical data: experimental units (level 1) nested in blocks (level 2)
   ...
   - The higher the ICC, the larger the loss in power when blocks are ignored

- Replicated randomized block design
  - Three-level hierarchical data: experimental units (level 1) nested in treatment conditions (level 2) nested in blocks (level 3)
  - Ignoring the treatment–block interaction pools block-specific treatment differences into the fixed treatment effect estimates, resulting in standard errors that are too small (anti-conservative)
  - The higher the treatment-block ICC, the larger the inflation of Type I error when the interaction is ignored

- Inter-rater agreement
  - Cross-classified data: measurements (level 1) cross-classified by subject (level 2) and rater (level 2)
  - Inter-rater agreement is quantified by the ICC for subjects
    - A high subject ICC indicates that measurements within the same subject are highly correlated (raters give similar scores to the same subject)

:::

## ICC versus explained variance

::: {style="font-size: 0.88em;"}

- **ICC** describes the correlation structure in the data (how similar are units within the same group?)
  - Relevant for: understanding dependence, sample size calculations

- **Explained variance (R-squared)** describes how well covariates predict the outcome
  - Relevant for: assessing model fit, evaluating the importance of predictors

#### In multi-level models

- **R-squared has two forms**
  - Marginal R-squared: proportion of variance explained by fixed effects only (similar to OLS)
  - Conditional R-squared: proportion of variance explained by both fixed and random effects

- **ICC can be calculated from different models**
  - ICC from null model (no covariates): describes the raw clustering structure in the data
    - In observational studies: motivates the use of multi-level models and quantifies dependence before any adjustments
  - ICC from treatment model (includes treatment as fixed effect): describes the residual clustering after adjusting for treatment (clustering among units with identical treatment assignment)
    - In experimental studies: determines power and sample size requirements
...
$$\text{ICC}_{\text{neighborhood}} = \frac{\sigma^2_{\text{district}} + \sigma^2_{\text{neighborhood:district}}}{\sigma^2_{\text{district}} + \sigma^2_{\text{neighborhood:district}} + \sigma^2} = `r round(icc_neighborhood_3lvl, 3)`$$

$$\text{ICC}_{\text{district}} = \frac{\sigma^2_{\text{district}}}{\sigma^2_{\text{district}} + \sigma^2_{\text{neighborhood:district}} + \sigma^2} = `r round(icc_district, 3)`$$
:::
...
## [Comparing estimated variance components]{style="font-size: 0.9em;"}
...
::: {style="margin-top:1.0em; font-size: 0.85em;"}
