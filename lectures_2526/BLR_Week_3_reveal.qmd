...
- Research questions:
  - To what extent does length of stay vary between hospitals compared with within hospitals?
...
## [Exploratory data analysis - variance decomposition]{style="font-size: 0.9em;"}
...
- This "unconditional" ICC quantifies clustering before accounting for any patient or hospital characteristics
...
- Centering means subtracting the mean from each observation: $X_{\text{centered}} = X - \bar{X}$
- Benefits of centering:
  - Improves interpretation: intercept represents outcome at the mean of covariates (rather than at zero, which may be outside the observed range)
  - Reduces multicollinearity when including interaction terms
...
$$
Y_{ij} = \mu + \beta_1 \text{Age_centered}_{ij} + \beta_2 \text{CCI_centered}_{ij} + \alpha_j + \beta_3 \text{Size}_j + u_j + \varepsilon_{ij}
$$
...
- After adjusting for patient characteristics, academic hospitals have a mean length of stay of `r round(summary(emm_academic)$emmean[1], 2)` days, while non-academic hospitals have `r round(summary(emm_academic)$emmean[2], 2)` days
...
vc_comparison <- data.frame(
  Model = c("Null model", "Null model", "Patient-level characteristics", "Patient-level characteristics", "Full model", "Full model"),
...
- **Unconditional ICC** (null model) = `r round(as.data.frame(VarCorr(model_null))$vcov[1] / sum(as.data.frame(VarCorr(model_null))$vcov), 3)`: `r round(100 * as.data.frame(VarCorr(model_null))$vcov[1] / sum(as.data.frame(VarCorr(model_null))$vcov), 0)`% of total variance in length of stay is attributable to differences between hospitals before adjusting for any covariates

- Adding patient-level characteristics reduces **residual variance** (within-hospital variation)

- Adding hospital-level characteristics reduces **hospital variance** (between-hospital variation)

- **Conditional ICC** (full model) = `r round(as.data.frame(VarCorr(model_full))$vcov[1] / sum(as.data.frame(VarCorr(model_full))$vcov), 3)`: after accounting for all covariates, `r round(100 * as.data.frame(VarCorr(model_full))$vcov[1] / sum(as.data.frame(VarCorr(model_full))$vcov), 0)`% of unexplained variance remains at the hospital level

## Unconditional versus conditional ICC

- Unconditional ICC
  - Computed from a null model without covariates
  - Captures the raw intra-cluster correlation in the data
  - Useful for describing the dependence structure in cross-sectional, hierarchical datasets

- Conditional ICC
  - Computed from a model that includes covariates
  - Reflects residual clustering after adjustment
  - Influences precision differently depending on the type of fixed effect
    - Cluster-level effects: higher ICC → lower precision
    - Individual-level effects: higher ICC → higher precision
